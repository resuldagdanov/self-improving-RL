...
if __name__ == "__main__":
    ...
    # organize parameters
    env, general_config, improvement_config = training_utils.initialize_config(
        env_config_path="/env_config.yaml",
        model_config_path="/ppo_config.yaml",
        train_config_path="/self_improvement.yaml"
    )
    ...
    if improvement_config["is_restore"] is True:
        checkpoint_num = improvement_config["checkpoint_number"]

        agent_path = os.path.join(training_utils.repo_path, "results/trained_models/" + improvement_config["load_agent_name"])
    ...
    for _ in range(improvement_config["stop"]["training_iteration"]):
